...
import { activateTab, getActiveTab, onTabChange } from './tabs.js';
...
activateTab(initialTab, { force: true, skipAnimation: true });

let hasProcessedInitialTabChange = false;
onTabChange(name => {
  if (!hasProcessedInitialTabChange) {
    hasProcessedInitialTabChange = true;
    return;
  }
  if (!name) return;
  const activeButton = qs(`.tab[data-go="${name}"]`);
  const iconContainer = activeButton?.querySelector('.tab__icon');
  if (iconContainer) triggerTabIconAnimation(iconContainer);
});
