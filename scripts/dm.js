...
const DM_CREDENTIAL_USERNAME = DM_CREDENTIAL_DEFAULT_USERNAME;
...
function initDMLogin(){
  ...
  async function beginCreatePinFlow() {
    try {
      await refreshDmCredentialCache();
    } catch (error) {
      console.warn('Failed to refresh DM credential cache before PIN setup', error);
    }
    let existing = false;
    try {
      existing = await hasDmCredential({ username: DM_CREDENTIAL_USERNAME });
    } catch (error) {
      console.error('Failed to determine DM credential status', error);
    }
    loginFlowState.mode = existing ? 'reset' : 'create';
    showCreatePinView();
  }
  ...
  async function completePinCreation() {
    ...
    const mode = (loginFlowState.mode === 'create' || loginFlowState.mode === 'reset')
      ? loginFlowState.mode
      : (isLoggedIn() ? 'reset' : 'create');
    const allowOverwrite = mode === 'reset';
    const confirmButton = loginConfirmSubmit || null;
    const confirmInput = loginConfirmPin || null;
    ...
    if (confirmInput) {
      ...
      confirmInput.setAttribute('aria-disabled', 'true');
    }
    let saved = false;
    try {
      await setDmCredentialPin({
        username: DM_CREDENTIAL_USERNAME,
        pin: finalPin,
        allowOverwrite,
      });
      await refreshDmCredentialCache();
      ...
      console.error('Failed to save DM PIN', error);
      ...
      if (confirmInput) {
        ...
        confirmInput.removeAttribute('aria-disabled');
      }
    }
    if (!saved) {
      if (loginConfirmPin && typeof loginConfirmPin.select === 'function') {
    ...
    dispatchLoginEvent('dm-login:set-pin', { pin: finalPin, mode });
    ...
    try {
      toast(mode === 'reset' ? 'DM PIN updated' : 'DM PIN created', 'success');
    } catch (error) {
      console.warn('Failed to display DM PIN notification', error);
    }
  }
  ...
  if (loginSetPinTriggers.length > 0) {
    loginSetPinTriggers.forEach(btn => {
      btn?.addEventListener('click', event => {
        ...
        beginCreatePinFlow().catch(error => {
          console.error('Failed to start DM PIN creation flow', error);
          showLoginError('Unable to start PIN setup. Try again.');
        });
      });
    });
  }
  ...
  if (loginConfirmSubmit) {
    loginConfirmSubmit.addEventListener('click', async event => {
      try { event?.preventDefault?.(); } catch { /* ignore */ }
      await completePinCreation();
    });
  }
  if (loginNewPin) {
  ...
  if (loginConfirmPin) {
    loginConfirmPin.addEventListener('keydown', async event => {
      if (event?.key === 'Enter') {
        try { event.preventDefault(); } catch { /* ignore */ }
        await completePinCreation();
      }
