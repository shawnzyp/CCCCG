...
const CLOUD_CONFLICTS_URL = `${CLOUD_HISTORY_URL}/conflicts`;
...
function getUserUrls(uid) {
  ...
  return {
    ...
    autosavesIndexUrl: `${CLOUD_USERS_URL}/${encodedUid}/autosaves`,
  };
}
...
export async function saveCloudAutosave(name, payload) {
  ...
  try {
    ...
    await saveAutosaveIndex(uid, autosaveKey, ts);
    resetOfflineNotices();
...
export async function saveCloudCharacter(uid, characterId, payload) {
  ...
  const cloudPayload = payload && typeof payload === 'object'
    ? {
        ...payload,
        meta: {
          ...(payload.meta && typeof payload.meta === 'object' ? payload.meta : {}),
    ...
    : payload;
  const serialized = safeJsonStringify(cloudPayload);
  if (!serialized.ok) {
    const error = new Error('Invalid JSON payload for cloud save');
    error.name = 'InvalidPayloadError';
    error.cause = serialized.error;
...
export async function saveAutosaveIndex(uid, characterId, latestTs) {
  if (typeof fetch !== 'function') throw new Error('fetch not supported');
  const urls = getUserUrls(uid);
  if (!urls) throw new Error('Missing user id');
  const encodedCharacterId = encodePath(characterId || '');
  ...
  const payload = {
    latestTs: Number(latestTs) || 0,
  };
  const url = await withAuth(`${urls.autosavesIndexUrl}/${encodedCharacterId}.json`);
  const res = await cloudFetch(url, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
export async function listAutosaveIndex(uid) {
  try {
    if (typeof fetch !== 'function') throw new Error('fetch not supported');
    const urls = getUserUrls(uid);
    if (!urls) return [];
    const url = await withAuth(`${urls.autosavesIndexUrl}.json`);
    const res = await cloudFetch(url);
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const val = await res.json();
    if (!val || typeof val !== 'object') return [];
    return Object.entries(val).map(([characterId, entry]) => ({
      characterId,
      latestTs: Number(entry?.latestTs) || 0,
    }));
  } catch (err) {
    if (err && err.message === 'fetch not supported') {
      throw err;
    }
    console.error('Autosave index list failed', err);
    return [];
  }
}

export async function saveConflictBackup(uid, characterId, payload) {
  if (typeof fetch !== 'function') throw new Error('fetch not supported');
  if (!uid || !characterId) throw new Error('Missing conflict metadata');
  const ts = nextHistoryTimestamp();
  const baseUrl = `${CLOUD_CONFLICTS_URL}/${encodePath(uid)}`;
  await saveHistoryEntry(baseUrl, characterId, payload, ts);
  return ts;
}
...
export async function listCloudBackups(name) {
  try {
    if (typeof fetch !== 'function') throw new Error('fetch not supported');
    const urls = getCloudUrls();
    const url = await withAuth(`${urls.historyUrl}/${encodePath(name)}.json`);
    const res = await cloudFetch(url);
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const val = await res.json();
    return val
...
export async function listCloudBackupNames() {
  try {
    if (typeof fetch !== 'function') throw new Error('fetch not supported');
    const urls = getCloudUrls();
    const url = await withAuth(`${urls.historyUrl}.json`);
    const res = await cloudFetch(url);
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const val = await res.json();
    return val ? Object.keys(val).map(k => decodePath(k)) : [];
...
export async function listCloudAutosaveNames() {
  try {
    ...
    const uid = activeAuthUserId;
    if (!uid) return [];
    const indexEntries = await listAutosaveIndex(uid);
    const names = [];
    for (const entry of indexEntries) {
      if (!entry?.characterId || !entry.latestTs) continue;
      try {
        const payload = await loadCloudAutosave('', entry.latestTs, { characterId: entry.characterId });
        const name = payload?.meta?.name;
        if (typeof name === 'string' && name.trim()) {
          names.push(name.trim());
        }
      } catch (err) {
        console.warn('Failed to read autosave for index entry', err);
      }
    }
    return names;
  } catch (e) {
...
export async function loadCloudBackup(name, ts) {
  try {
    if (typeof fetch !== 'function') throw new Error('fetch not supported');
    const urls = getCloudUrls();
    const url = await withAuth(`${urls.historyUrl}/${encodePath(name)}/${ts}.json`);
    const res = await cloudFetch(url, { method: 'GET' });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const val = await res.json();
    if (val !== null) return val;
  } catch (e) {
